<template>
  <cms-content
    title-page="Энергетические напитки"
...
export default Vue.extend({
  name: "cms-categories-beverages-energetic-drinks",
  layout: 'cms',
  async asyncData({ store }) {
    await store.dispatch('database/categories/beverages/energetic-drinks/getAll');
  },
  data() {
    return {
      formData: {
  ...
  methods: {
    ...
    replaceTextBr,
    async addCategory() {
      return await this.$store.dispatch('database/categories/beverages/energetic-drinks/add', this.formData);
    },
    async editCategory() {
      return await this.$store.dispatch('database/categories/beverages/energetic-drinks/update', this.formData);
    },
    async removeCategory(id) {
      await this.$store.dispatch('database/categories/beverages/energetic-drinks/remove', id);
    },
    async moveCategory({ newPos, el }) {
      await this.$store.dispatch('database/categories/beverages/energetic-drinks/move', { newPos, el });
    },
  ...
  computed: {
    ...mapGetters({
      categories: 'database/categories/beverages/energetic-drinks/getAll'
    })
  }
})
</script>
