...
export default Vue.extend({
  name: "cms-categories-tea-сlassic",
  layout: 'cms',
  async asyncData({ store }) {
    await store.dispatch('database/categories/tea/сlassic/getAll');
  },
  data() {
    return {
      formData: {
  ...
  methods: {
    ...
    replaceTextBr,
    async addCategory() {
      return await this.$store.dispatch('database/categories/tea/сlassic/add', this.formData);
    },
    async editCategory() {
      return await this.$store.dispatch('database/categories/tea/сlassic/update', this.formData);
    },
    async removeCategory(id) {
      await this.$store.dispatch('database/categories/tea/сlassic/remove', id);
    },
    async moveCategory({ newPos, el }) {
      await this.$store.dispatch('database/categories/tea/сlassic/move', { newPos, el });
    },
  ...
  computed: {
    ...mapGetters({
      categories: 'database/categories/tea/сlassic/getAll'
    })
  }
})
</script>
