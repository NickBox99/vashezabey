...
export default Vue.extend({
  name: "cms-categories-tea-additives",
  layout: 'cms',
  async asyncData({ store }) {
    await store.dispatch('database/categories/tea/additives/getAll');
  },
  data() {
    return {
      formData: {
  ...
  methods: {
    ...
    replaceTextBr,
    async addCategory() {
      return await this.$store.dispatch('database/categories/tea/additives/add', this.formData);
    },
    async editCategory() {
      return await this.$store.dispatch('database/categories/tea/additives/update', this.formData);
    },
    async removeCategory(id) {
      await this.$store.dispatch('database/categories/tea/additives/remove', id);
    },
    async moveCategory({ newPos, el }) {
      await this.$store.dispatch('database/categories/tea/additives/move', { newPos, el });
    },
  ...
  computed: {
    ...mapGetters({
      categories: 'database/categories/tea/additives/getAll'
    })
  }
})
</script>
