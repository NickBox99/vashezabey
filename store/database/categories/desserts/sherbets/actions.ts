...
export default {
  async getAll({ dispatch }: Database.IStore): Promise<Database.ICategory[]> {
    return dispatch('cache/getUseCache', { key: 'categories-desserts-beer', fetchCallback: () => Vue.prototype.$fb.categories.desserts.beer.getAll() }, { root: true })
  },
  ...
  async add({ commit }: Database.IStore, category: Database.ICategory) {
    commit('cache/add', { key: 'categories-desserts-beer', value: await Vue.prototype.$fb.categories.desserts.beer.add(category) }, { root: true });
  },
  ...
  async update({ commit, dispatch }: Database.IStore, category: Database.ICategory) {
    ...
    commit('cache/update', { key: 'categories-desserts-beer', value: newCategory}, { root: true });
    Vue.prototype.$fb.categories.desserts.beer.update(newCategory);
    return true;
  },

  async remove({ commit }: Database.IStore, id: string) {
    await Vue.prototype.$fb.categories.desserts.beer.remove(id);
    commit('cache/remove', { key: 'categories-desserts-beer', id }, { root: true });
  },
