...
export default {
  async getAll({ dispatch }: Database.IStore): Promise<Database.ICategory[]> {
    return dispatch('cache/getUseCache', { key: 'categories-desserts-milkshakes', fetchCallback: () => Vue.prototype.$fb.categories.desserts.milkshakes.getAll() }, { root: true })
  },
  ...
  async add({ commit }: Database.IStore, category: Database.ICategory) {
    commit('cache/add', { key: 'categories-desserts-milkshakes', value: await Vue.prototype.$fb.categories.desserts.milkshakes.add(category) }, { root: true });
  },
  ...
  async update({ commit, dispatch }: Database.IStore, category: Database.ICategory) {
    ...
    commit('cache/update', { key: 'categories-desserts-milkshakes', value: newCategory}, { root: true });
    Vue.prototype.$fb.categories.desserts.milkshakes.update(newCategory);
    return true;
  },

  async remove({ commit }: Database.IStore, id: string) {
    await Vue.prototype.$fb.categories.desserts.milkshakes.remove(id);
    commit('cache/remove', { key: 'categories-desserts-milkshakes', id }, { root: true });
  },
