...
export default {
  async getAll({ dispatch }: Database.IStore): Promise<Database.ICategory[]> {
    return dispatch('cache/getUseCache', { key: 'beveragesBeer', fetchCallback: () => Vue.prototype.$fb.categories.beverages.beer.getAll() }, { root: true })
  },
  ...
  async add({ commit }: Database.IStore, category: Database.ICategory) {
    const result: Database.ICategory | null = await Vue.prototype.$fb.categories.beverages.beer.add(category);

    if (result) {
      commit('cache/add', { key: 'beveragesBeer', value: result }, { root: true });
      return true;
    }
    else {
      return false;
  ...
  async update({ commit, dispatch }: Database.IStore, category: Database.ICategory) {
    ...
    commit('cache/update', { key: 'beveragesBeer', value: newCategory}, { root: true });
    await Vue.prototype.$fb.categories.beverages.beer.update(newCategory);
    return true;
  },

  async remove({ commit }: Database.IStore, id: string) {
    await Vue.prototype.$fb.categories.beverages.beer.remove(id);
    commit('cache/remove', { key: 'beveragesBeer', id }, { root: true });
    return true;
  },
