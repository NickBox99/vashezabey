...
export default {
  async getAll({ dispatch }: Database.IStore): Promise<Database.ICategory[]> {
    return dispatch('cache/getUseCache', { key: 'categories-beverages-beer', fetchCallback: () => Vue.prototype.$fb.categories.beverages.beer.getAll() }, { root: true })
  },
  ...
  async add({ commit }: Database.IStore, category: Database.ICategory) {
    commit('cache/add', { key: 'categories-beverages-beer', value: await Vue.prototype.$fb.categories.beverages.beer.add(category) }, { root: true });
  },
  ...
  async update({ commit, dispatch }: Database.IStore, category: Database.ICategory) {
    ...
    commit('cache/update', { key: 'categories-beverages-beer', value: newCategory}, { root: true });
    Vue.prototype.$fb.categories.beverages.beer.update(newCategory);
    return true;
  },

  async remove({ commit }: Database.IStore, id: string) {
    await Vue.prototype.$fb.categories.beverages.beer.remove(id);
    commit('cache/remove', { key: 'categories-beverages-beer', id }, { root: true });
  },
