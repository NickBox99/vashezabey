...
export default {
  async getAll({ dispatch }: Database.IStore): Promise<Database.ICategory[]> {
    return dispatch('cache/getUseCache', { key: 'hookahFruity', fetchCallback: () => Vue.prototype.$fb.categories.hookah.fruity.getAll() }, { root: true })
  },
  ...
  async add({ commit }: Database.IStore, category: Database.ICategory) {
    const result: Database.ICategory | null = await Vue.prototype.$fb.categories.hookah.fruity.add(category);

    if (result) {
      commit('cache/add', { key: 'hookahFruity', value: result }, { root: true });
      return true;
    }
    else {
      return false;
  ...
  async update({ commit, dispatch }: Database.IStore, category: Database.ICategory) {
    ...
    commit('cache/update', { key: 'hookahFruity', value: newCategory}, { root: true });
    await Vue.prototype.$fb.categories.hookah.fruity.update(newCategory);
    return true;
  },

  async remove({ commit }: Database.IStore, id: string) {
    await Vue.prototype.$fb.categories.hookah.fruity.remove(id);
    commit('cache/remove', { key: 'hookahFruity', id }, { root: true });
    return true;
  },
