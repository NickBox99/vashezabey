...
export default {
  async getAll({ dispatch }: Database.IStore): Promise<Database.ICategory[]> {
    return dispatch('cache/getUseCache', { key: 'hookahCorkageFees', fetchCallback: () => Vue.prototype.$fb.categories.hookah.corkageFees.getAll() }, { root: true })
  },
  ...
  async add({ commit }: Database.IStore, category: Database.ICategory) {
    const result: Database.ICategory | null = await Vue.prototype.$fb.categories.hookah.corkageFees.add(category);

    if (result) {
      commit('cache/add', { key: 'hookahCorkageFees', value: result }, { root: true });
      return true;
    }
    else {
      return false;
  ...
  async update({ commit, dispatch }: Database.IStore, category: Database.ICategory) {
    ...
    commit('cache/update', { key: 'hookahCorkageFees', value: newCategory}, { root: true });
    await Vue.prototype.$fb.categories.hookah.corkageFees.update(newCategory);
    return true;
  },

  async remove({ commit }: Database.IStore, id: string) {
    await Vue.prototype.$fb.categories.hookah.corkageFees.remove(id);
    commit('cache/remove', { key: 'hookahCorkageFees', id }, { root: true });
    return true;
  },
