...
export default {
  async getAll({ dispatch }: Database.IStore): Promise<Database.ICategory[]> {
    return dispatch('cache/getUseCache', { key: 'teaClassic', fetchCallback: () => Vue.prototype.$fb.categories.tea.сlassic.getAll() }, { root: true })
  },
  ...
  async add({ commit }: Database.IStore, category: Database.ICategory) {
    const result: Database.ICategory | null = await Vue.prototype.$fb.categories.tea.сlassic.add(category);

    if (result) {
      commit('cache/add', { key: 'teaClassic', value: result }, { root: true });
      return true;
    }
    else {
      return false;
  ...
  async update({ commit, dispatch }: Database.IStore, category: Database.ICategory) {
    ...
    commit('cache/update', { key: 'teaClassic', value: newCategory}, { root: true });
    await Vue.prototype.$fb.categories.tea.сlassic.update(newCategory);
    return true;
  },

  async remove({ commit }: Database.IStore, id: string) {
    await Vue.prototype.$fb.categories.tea.сlassic.remove(id);
    commit('cache/remove', { key: 'teaClassic', id }, { root: true });
    return true;
  },
